        ...
        <div class="session-panel">
          ...
          <div class="session-list">
            <div v-for="(item, index) in exampleTextList" :key="index" class="exampleTextListClass"
                 @click="printExampleText(item.exampleText)">
          ...
          <div class="more-functions">
            <div class="model-select" @click="showOptions">
              <span style="margin-right: 50px">模型选择</span>
              <el-button @click="selectModel">{{modelType}}</el-button>
            </div>
          </div>
        </div>
          ...
          <div class="chatMessage">
            <div class="chatBox" id="chatBox">
              <el-row v-for="(item, index) in responseText" :key="index"
                      :class="[item.is_robot === 0 ? 'robot' : 'user']">
                <el-col :span="2">
                  <img
                    :src="item.is_robot === 0 ? require('@/assets/image/aiLogo.jpg') : require('@/assets/image/user.jpg')"
                    style="width: 30px; height: 30px;">
                </el-col>
                <el-col :span="22" >
                  <div style="height: 100%;width: 100%;" v-if="item.is_robot !== 0">{{item.content}}</div>
                  <div style="height: 100%;width: 100%;" v-else >
                    ...
                    <div style="height: 100%;width: 100%;margin-bottom: 5px" v-for="item in item.content">
                        ...
                        <span><a :href="item.href" target="_blank" rel="noopener noreferrer">查看详情</a></span>
                    </div>
                  </div>
                </el-col>
      ...
      async sendKeyBert() {
        ...
        getRecommendData(inputValue).then(res => {
          if (res.code === 20000) {
            ...
            console.log(res.data.map(item => item.position))
            if (res.data.length < 1) {
              return Message({
                message: '尊敬的用户，请重新换种输入内容！',
                type: 'warning'
              })
            } else {
